...
#include <chrono>
#include <tlhelp32.h>
#include "stream_observation.hpp"
namespace kknet {
...
struct SocketState {
    kkobserve::Stream observed_decoded,observed_delivered;
...
inline int WSAAPI recv_hook(SOCKET s,char* data,int n,int flags){
    ++kkobserve::api[kkobserve::Recv];if(flags==MSG_PEEK)++kkobserve::api[kkobserve::Peek];
    CallScope scope;
    try{
        if(n<0||(flags!=0&&flags!=MSG_PEEK)||(!data&&n))return error(WSAEINVAL);
        if(flags==MSG_PEEK&&static_cast<size_t>(n)>queue_limit)return error(WSAEMSGSIZE);
        auto p=state(s,false);if(!p)return SOCKET_ERROR;if(!n)return 0;Guard lock(p->rx);
        if(flags==MSG_PEEK&&p->plain_at&&p->plain.size()-p->plain_at<static_cast<size_t>(n)){
            size_t left=p->plain.size()-p->plain_at;
            std::memmove(p->plain.data(),p->plain.data()+p->plain_at,left);
            SecureZeroMemory(p->plain.data()+left,p->plain_at);p->plain.resize(left);p->plain_at=0;
        }
        // Cipher records are not application stream boundaries. Once some
        // plaintext is buffered, only drain immediately readable ciphertext;
        // a partial next record must not block a peek of the existing bytes.
        auto next_record=[&](bool block)->int{for(;;){
            ...
            if(p->wire.size()==needed&&needed>kkrecord::header_size){
                size_t count=needed-kkrecord::header_size-16;
                if(p->plain.size()+count>queue_limit+kkrecord::max_plain)return error(WSAENOBUFS);
                p->plain.reserve(p->plain.size()+count);std::vector<unsigned char> decoded;
                if(!p->records.open(p->wire.data(),p->wire.size(),decoded)){++kkobserve::api[kkobserve::DecryptFailure];p->dead=true;return error(WSAECONNRESET);}
                if(p->records.channel==2)p->observed_decoded.feed(decoded.data(),decoded.size(),kkobserve::decoded);
                p->plain.insert(p->plain.end(),decoded.begin(),decoded.end());SecureZeroMemory(decoded.data(),decoded.size());p->wire.clear();return 1;
            }
            if(!block){
                fd_set read;FD_ZERO(&read);FD_SET(s,&read);timeval zero{};
                int ready=raw_select(0,&read,nullptr,nullptr,&zero);
                if(ready<0)return SOCKET_ERROR;if(!ready)return error(WSAEWOULDBLOCK);
            }
        ...
        if(p->plain.empty()){int result=next_record(true);if(result<=0)return result;}
        if(flags==MSG_PEEK){while(p->plain.size()-p->plain_at<static_cast<size_t>(n)){
            int result=next_record(false);
            if(result==0)break;
            if(result<0){if(WSAGetLastError()==WSAEWOULDBLOCK)break;return SOCKET_ERROR;}
        }}
        ...
        if(flags!=MSG_PEEK&&p->records.channel==2)p->observed_delivered.feed(data,static_cast<size_t>(count),kkobserve::delivered);
...
inline int WSAAPI recvfrom_hook(SOCKET s,char* data,int n,int flags,sockaddr* from,int* size){
    ++kkobserve::api[kkobserve::RecvFrom];int socket_type=0,type_size=sizeof(socket_type);
    if(!getsockopt(s,SOL_SOCKET,SO_TYPE,reinterpret_cast<char*>(&socket_type),&type_size)&&socket_type==SOCK_STREAM)++kkobserve::api[kkobserve::RecvFromTcp];
    CallScope scope;try{
...
inline int WSAAPI ioctls_hook(SOCKET s,long cmd,u_long* value){
    if(cmd==FIONREAD)++kkobserve::api[kkobserve::Fionread];
