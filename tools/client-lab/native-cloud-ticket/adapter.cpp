...
extern "C" __declspec(dllexport) DWORD WINAPI KkNativeCloudInstall(void*){
    ...
    try{
        ...
        if(!kknet::collect(plan))return install_failure(kknet::coverage_diagnostic.reason==24?kknet::coverage_diagnostic.location:ERROR_NOT_SUPPORTED);
...
#pragma pack(pop)
extern "C" __declspec(dllexport) DWORD WINAPI KkNativeCloudGetCoverageDiagnostic(kknet::CoverageDiagnostic* output){
    kknet::CoverageDiagnostic input{};SIZE_T got=0;
    if(!output||!ReadProcessMemory(GetCurrentProcess(),output,&input,sizeof(input),&got)||got!=sizeof(input)||input.size!=sizeof(input)||input.version!=1)return ERROR_INVALID_PARAMETER;
    std::lock_guard<std::mutex> lock(kkhooks::lifecycle);SIZE_T written=0;
    return WriteProcessMemory(GetCurrentProcess(),output,&kknet::coverage_diagnostic,sizeof(input),&written)&&written==sizeof(input)?ERROR_SUCCESS:ERROR_INVALID_PARAMETER;
}
extern "C" __declspec(dllexport) DWORD WINAPI KkNativeCloudGetIoDiagnostic(kkobserve::Snapshot* output){
    kkobserve::Snapshot input{};SIZE_T got=0;
    if(!output||!ReadProcessMemory(GetCurrentProcess(),output,&input,sizeof(input),&got)||got!=sizeof(input)||input.size!=sizeof(input)||input.version!=1)return ERROR_INVALID_PARAMETER;
    const auto result=kkobserve::snapshot();SIZE_T written=0;
...
int main(int argc,char** argv){
    ...
    {
        auto frame=[](uint32_t id){std::array<unsigned char,24> b{};uint16_t magic=0xaaee,mask=(16^0xbbcc)&0x88aa;uint32_t size=16;
            std::memcpy(b.data(),&magic,2);std::memcpy(b.data()+2,&mask,2);std::memcpy(b.data()+4,&size,4);
            uint64_t key=0;std::memcpy(&key,"00Na~1fd",8);uint64_t encoded=(uint64_t(id)<<3)^key;
            std::memcpy(b.data()+8,&encoded,8);std::memcpy(b.data()+16,&key,8);return b;};
        auto f=frame(1151);
        for(size_t split=0;split<=f.size();split++){
            kkobserve::Stream observer;std::array<std::atomic<uint32_t>,12> counts{};
            observer.feed(f.data(),split,counts);observer.feed(f.data()+split,f.size()-split,counts);assert(counts[6]==1&&!observer.disabled);
        }
        kkobserve::Stream observer;std::array<std::atomic<uint32_t>,12> counts{};
        auto unknown=frame(99999);observer.feed(unknown.data(),unknown.size(),counts);for(auto& n:counts)assert(n==0);
        auto zero=frame(0);for(auto byte:zero)observer.feed(&byte,1,counts);assert(counts[0]==1);
        kkobserve::Snapshot output;assert(KkNativeCloudGetIoDiagnostic(&output)==0&&output.size==136);output.version=99;assert(KkNativeCloudGetIoDiagnostic(&output)==ERROR_INVALID_PARAMETER);
        puts("PASS: metadata-only stream observer; all header splits; unknown-ID finite labels; diagnostic ABI.");
    }
    ...
    if(argc==3&&std::strcmp(argv[1],"--peek-records")==0){
        int port=std::atoi(argv[2]);assert(port>1024&&port<65535);c.game_port=static_cast<unsigned short>(port);c.sdk_port=static_cast<unsigned short>(port-1);
        assert(KkNativeCloudSetTicket(&c)==0);SOCKET s=socket(AF_INET,SOCK_STREAM,IPPROTO_TCP);assert(s!=INVALID_SOCKET);
        ...
        auto wait_raw=[&](u_long minimum){auto until=GetTickCount64()+5000;u_long available=0;do{assert(ioctlsocket(s,FIONREAD,&available)==0);if(available>=minimum)return;Sleep(1);}while(GetTickCount64()<until);assert(false);};
        assert(kknet::send_hook(s,"peek",4,0)==4);wait_raw(104);
        char out[8]{};assert(kknet::recv_hook(s,out,8,MSG_PEEK)==4&&!std::memcmp(out,"head",4));
        assert(kknet::send_hook(s,"next",4,0)==4);wait_raw(40);
        assert(kknet::recv_hook(s,out,8,MSG_PEEK)==8&&!std::memcmp(out,"headtail",8));
        assert(kknet::recv_hook(s,out,8,MSG_PEEK)==8&&!std::memcmp(out,"headtail",8));
        assert(kknet::recv_hook(s,out,5,0)==5&&!std::memcmp(out,"headt",5));
        assert(kknet::recv_hook(s,out,8,0)==3&&!std::memcmp(out,"ail",3));
        assert(kknet::recv_hook(s,out,1,0)==0);assert(kknet::close_hook(s)==0);kknet::retire();WSACleanup();
        puts("PASS: MSG_PEEK grows across encrypted records and partial header; repeated peek, split consume and EOF.");return 0;
    }
    ...
    kknet::CoverageDiagnostic diagnostic{};assert(KkNativeCloudGetCoverageDiagnostic(&diagnostic)==0&&diagnostic.reason==1);
    diagnostic.size=0;assert(KkNativeCloudGetCoverageDiagnostic(&diagnostic)==ERROR_INVALID_PARAMETER);
