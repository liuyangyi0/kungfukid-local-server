...
static void hook_transaction_contract(){
    ...
    auto pause=[&](DWORD ms){assert(ms==50);++pauses;};
    auto found=reinterpret_cast<HANDLE>(42);
    assert(module_snapshot([&]{return ++attempts==3?found:INVALID_HANDLE_VALUE;},[]{return DWORD(ERROR_BAD_LENGTH);},pause)==found&&attempts==3&&pauses==2);
    for(DWORD code:{DWORD(ERROR_BAD_LENGTH),DWORD(ERROR_ACCESS_DENIED),DWORD(ERROR_PARTIAL_COPY)}){
        attempts=0;pauses=0;
        assert(module_snapshot([&]{++attempts;return INVALID_HANDLE_VALUE;},[&]{return code;},pause)==INVALID_HANDLE_VALUE);
        assert(GetLastError()==code&&attempts==(code==ERROR_BAD_LENGTH?8u:1u)&&pauses==(code==ERROR_BAD_LENGTH?7u:0u));
    }
    // Import presence is not permission to accept unencrypted peer sockets.
    HMODULE ws=GetModuleHandleW(L"WS2_32.dll");assert(ws);
    for(const auto& item:std::array<std::pair<const char*,WORD>,2>{{{"accept",WORD(1)},{"listen",WORD(13)}}}){
        auto real=reinterpret_cast<void*>(GetProcAddress(ws,item.first));assert(real);
        assert(real==reinterpret_cast<void*>(GetProcAddress(ws,MAKEINTRESOURCEA(item.second))));
        auto adapted=kknet::resolve(ws,real);assert(adapted&&adapted!=real);
        assert(reinterpret_cast<void*>(kknet::lookup_hook(ws,item.first))==adapted);
        if(item.second==1){
            sockaddr_in address{};address.sin_family=AF_INET;int length=sizeof(address);
            auto before=address;
            assert(reinterpret_cast<decltype(&::accept)>(adapted)(INVALID_SOCKET,reinterpret_cast<sockaddr*>(&address),&length)==INVALID_SOCKET);
            assert(WSAGetLastError()==WSAEOPNOTSUPP&&length==sizeof(address)&&!std::memcmp(&address,&before,sizeof(address)));
        }else{
            assert(reinterpret_cast<decltype(&::listen)>(adapted)(INVALID_SOCKET,1)==SOCKET_ERROR);
            assert(WSAGetLastError()==WSAEOPNOTSUPP);
        }
    }
    assert(!kknet::resolve(ws,reinterpret_cast<void*>(GetProcAddress(ws,"WSAIoctl"))));
