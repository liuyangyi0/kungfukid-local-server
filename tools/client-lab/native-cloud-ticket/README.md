...
Remove撤销凭据并恢复业务槽，但保留已pin的网络拒绝层到进程退出，不能靠卸载安全层恢复明文。
...
短写/WSAEWOULDBLOCK保留同一密文和原始输入，重试不得重新加密同序号；内部缓存就绪会补入`select`可读集合。
UDP只发往配置服务器端点，禁止原直连打洞；原1008中继仍由服务校验同房和租约后转发。
当前适配显式拒绝未支持的Winsock方法、overlapped与scatter/gather，不假装异步接口已兼容。
...
Apply保存原防火墙profile和原允许规则名称，再开启所有profile的默认拒绝出站，禁用既有出站允许规则。
...
规则持久保留，启动器崩溃不会自动撤销；恢复前必须关闭游戏，使用同一机器的原始收据显式Restore。
...
34项相关Python测试通过；CNG/Python固定向量一致，真实回环套接字完成100000字节加密往返、17字节读取、MSG_PEEK、缓存可读事件、强制短写/WouldBlock和UDP加解密。
