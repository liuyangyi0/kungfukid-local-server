...
2026-09-22公网模式更新：这是可选客户端实现，不是服务端认证前提。官方启动器不再强制VM、C:\KK-Lab或出站防火墙收据；路径由调用者传入。第三方可直接实现标准接口，见公开包docs/SERVICE.md。下文防火墙操作与原客户端隔离验收保留为历史工具说明，不属于公网接入门禁；本批不应用也不撤销任何真实系统规则。
