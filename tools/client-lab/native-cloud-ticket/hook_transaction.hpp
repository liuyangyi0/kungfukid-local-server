...
#include <mutex>
namespace kkhooks {
// Module loading can race a read-only Toolhelp snapshot. Bound the documented
// ERROR_BAD_LENGTH retry; access/architecture errors must not be hidden.
template<class Create,class Error,class Pause> HANDLE module_snapshot(Create create,Error error,Pause pause){
    for(unsigned attempt=0;attempt<8;attempt++){
        HANDLE h=create();if(h!=INVALID_HANDLE_VALUE)return h;
        DWORD code=error();if(code!=ERROR_BAD_LENGTH||attempt==7){SetLastError(code);return INVALID_HANDLE_VALUE;}
        pause(50);
    }
