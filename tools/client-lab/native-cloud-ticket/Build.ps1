...
$model=Join-Path $PSScriptRoot 'model_module.cpp'
...
$command+=' && cl /nologo /W4 /WX /utf-8 /std:c++17 /EHsc /MT /LD "'+$model+'" /Fo"'+$out+'\model.obj" /link /OUT:"'+$out+'\owned-model.dll" && "'+$out+'\contract.exe" --notification-model "'+$out+'\owned-model.dll"'
