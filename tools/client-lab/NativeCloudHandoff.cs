...
public sealed class NativeCloudOptions {
 ...
 public string certificate_path,initializer_log;
 public bool initializer_manual_start;
 ...
 public void Validate(){
  ...
  if(!String.IsNullOrEmpty(certificate_path)&&(!Path.IsPathRooted(certificate_path)||!File.Exists(certificate_path)))throw new InvalidOperationException("missing_server_certificate");
 }
}

// Only non-secret endpoint preferences are persisted. Tool paths and certificate
// trust remain installation-owned and cannot be changed by this preferences file.
public static class LauncherSettings {
 public static NativeCloudOptions Copy(NativeCloudOptions source){return new JavaScriptSerializer().Deserialize<NativeCloudOptions>(new JavaScriptSerializer().Serialize(source));}
 public static void Save(string path,NativeCloudOptions options){
  options.ValidateEndpoint();Directory.CreateDirectory(Path.GetDirectoryName(Path.GetFullPath(path)));
  string text=new JavaScriptSerializer().Serialize(new{schema="kk-launcher-endpoint-v1",auth_host=options.auth_host,sdk_host=options.sdk_host,auth_port=options.auth_port,sdk_port=options.sdk_port,game_port=options.game_port,udp_port=options.udp_port});
  string temp=path+"."+Guid.NewGuid().ToString("N")+".tmp";
  try{File.WriteAllText(temp,text,new UTF8Encoding(false));if(File.Exists(path))File.Replace(temp,path,null);else File.Move(temp,path);}finally{if(File.Exists(temp))File.Delete(temp);}
 }
 public static NativeCloudOptions Load(string path,NativeCloudOptions defaults){
  var result=Copy(defaults);if(!File.Exists(path))return result;
  if(new FileInfo(path).Length>4096)throw new InvalidOperationException("invalid_native_settings");
  var json=new JavaScriptSerializer();var row=json.Deserialize<Dictionary<string,object>>(File.ReadAllText(path));
  if(!row.ContainsKey("schema")||(string)row["schema"]!="kk-launcher-endpoint-v1")throw new InvalidOperationException("invalid_native_settings");
  var saved=json.Deserialize<NativeCloudOptions>(File.ReadAllText(path));saved.ValidateEndpoint();
  result.auth_host=saved.auth_host;result.sdk_host=saved.sdk_host;result.auth_port=saved.auth_port;result.sdk_port=saved.sdk_port;result.game_port=saved.game_port;result.udp_port=saved.udp_port;return result;
 }
 public static bool FreshInitializerReady(string path,long offset,int pid){
  if(String.IsNullOrEmpty(path)||!File.Exists(path))return false;
  using(var stream=new FileStream(path,FileMode.Open,FileAccess.Read,FileShare.ReadWrite)){
   if(offset<0||stream.Length<offset||stream.Length-offset>65536)throw new InvalidOperationException("initializer_log_bound");
   stream.Seek(offset,SeekOrigin.Begin);using(var reader=new StreamReader(stream)){
    string fresh=reader.ReadToEnd();int at=fresh.IndexOf(" loaded pid="+pid+" ",StringComparison.Ordinal);
    if(at<0)return false;string tail=fresh.Substring(at);
    return tail.IndexOf(" loaded pid=",1,StringComparison.Ordinal)<0&&tail.Contains(" roleprop_ready table=0x");
   }
  }
 }
 public static void Wait(Process process,int seconds,string error,Func<bool> ready){
  var clock=Stopwatch.StartNew();while(true){if(process.HasExited)throw new InvalidOperationException("native_client_exited");if(ready())return;if(clock.Elapsed.TotalSeconds>=seconds)throw new InvalidOperationException(error);Thread.Sleep(250);}
 }
}

public static class NativeCloudRpc {
 // Explicit per-connection certificate trust, never a global root-store edit.
 // Only an exact pinned leaf with a valid name/time and no other chain fault
 // can use an otherwise untrusted self-signed certificate.
 public static bool MatchPinnedCertificate(X509Certificate2 pinned,X509Certificate certificate,X509Chain chain,SslPolicyErrors errors){
  if(pinned==null||certificate==null||(errors&~SslPolicyErrors.RemoteCertificateChainErrors)!=0)return false;
  using(var actual=new X509Certificate2(certificate)){
   byte[] a=actual.RawData,b=pinned.RawData;if(a.Length!=b.Length)return false;int difference=0;for(int i=0;i<a.Length;i++)difference|=a[i]^b[i];if(difference!=0)return false;
   var now=DateTime.UtcNow;if(now<actual.NotBefore.ToUniversalTime()||now>actual.NotAfter.ToUniversalTime())return false;
   if(errors==SslPolicyErrors.None)return true;
   if(chain==null||chain.ChainElements.Count!=1||actual.Subject!=actual.Issuer)return false;
   foreach(var status in chain.ChainStatus)if((status.Status&~X509ChainStatusFlags.UntrustedRoot)!=0)return false;
   return true;
  }
 }
 public static object CallWithCertificate(string host,int port,string operation,Dictionary<string,object> arguments,string certificatePath){
  using(var pinned=new X509Certificate2(certificatePath))return CallCore(host,port,operation,arguments,null,(sender,certificate,chain,errors)=>MatchPinnedCertificate(pinned,certificate,chain,errors));
 }
 public static object CallConfigured(NativeCloudOptions options,string operation,Dictionary<string,object> arguments){
  if(String.IsNullOrEmpty(options.certificate_path))return Call(options.auth_host,options.auth_port,operation,arguments,options.sdk_host);
  using(var pinned=new X509Certificate2(options.certificate_path))return CallCore(options.auth_host,options.auth_port,operation,arguments,options.sdk_host,(sender,certificate,chain,errors)=>MatchPinnedCertificate(pinned,certificate,chain,errors));
 }
 ...
 public static object Call(string host,int port,string operation,Dictionary<string,object> arguments,string address=null){
  return CallCore(host,port,operation,arguments,address,null);
 }
 static object CallCore(string host,int port,string operation,Dictionary<string,object> arguments,string address,RemoteCertificateValidationCallback validation){
  ...
  try{using(var tcp=new TcpClient()){
   ...
   using(var tls=validation==null?new SslStream(tcp.GetStream(),false):new SslStream(tcp.GetStream(),false,validation)){
...
public static class NativeCloudHandoff {
 ...
 [StructLayout(LayoutKind.Sequential)]struct WindowPoint{public int x,y;}
 [DllImport("user32.dll")]static extern bool GetClientRect(IntPtr window,out WindowRect rect);
 [DllImport("user32.dll")]static extern bool ClientToScreen(IntPtr window,ref WindowPoint point);
 [DllImport("user32.dll")]static extern int GetSystemMetrics(int index);
 public static Dictionary<string,object> WindowFacts(int pid,string expectedExe){
  using(var p=Process.GetProcessById(pid)){
   if(!String.Equals(p.MainModule.FileName,expectedExe,StringComparison.OrdinalIgnoreCase))throw new InvalidOperationException("client_identity_mismatch");
   IntPtr w=p.MainWindowHandle;uint owner;if(w==IntPtr.Zero)return null;GetWindowThreadProcessId(w,out owner);if(owner!=(uint)pid)return null;
   WindowRect r;WindowPoint o=new WindowPoint();if(!GetClientRect(w,out r)||!ClientToScreen(w,ref o))return null;
   return new Dictionary<string,object>{{"hwnd",w.ToInt64().ToString()},{"client_rect",new Dictionary<string,int>{{"left",o.x},{"top",o.y},{"right",o.x+r.right},{"bottom",o.y+r.bottom}}},{"screen",new Dictionary<string,int>{{"virtual_left",GetSystemMetrics(76)},{"virtual_top",GetSystemMetrics(77)},{"virtual_width",GetSystemMetrics(78)},{"virtual_height",GetSystemMetrics(79)}}}};
  }
 }
 ...
 [DllImport("kernel32.dll",CharSet=CharSet.Unicode,SetLastError=true)]static extern bool Module32FirstW(IntPtr snapshot,ref ModuleEntry entry);
 [DllImport("kernel32.dll",CharSet=CharSet.Unicode,SetLastError=true)]static extern bool Module32NextW(IntPtr snapshot,ref ModuleEntry entry);
 ...
 // Loader churn may invalidate a Toolhelp snapshot (ERROR_BAD_LENGTH). Retry
 // only that documented transient failure; retain other native error codes.
 internal static IntPtr ModuleSnapshotWithRetry(uint pid,Func<uint,uint,IntPtr> create,Func<int> lastError,Action<int> pause){
  for(int attempt=0;attempt<8;attempt++){
   IntPtr snapshot=create(0x18,pid);if(snapshot!=new IntPtr(-1))return snapshot;
   int error=lastError();if(error!=24||attempt==7)throw new System.ComponentModel.Win32Exception(error,"native_modules_unavailable");
   pause(50);
  }
  throw new InvalidOperationException("native_modules_unavailable");
 }
 static IntPtr Module(int pid,string path){
  IntPtr h=ModuleSnapshotWithRetry((uint)pid,CreateToolhelp32Snapshot,Marshal.GetLastWin32Error,Thread.Sleep);
  try{ModuleEntry e=new ModuleEntry();e.size=(uint)Marshal.SizeOf(typeof(ModuleEntry));bool more=Module32FirstW(h,ref e);
   while(more){if(String.Equals(Path.GetFullPath(e.path),Path.GetFullPath(path),StringComparison.OrdinalIgnoreCase))return e.baseAddress;more=Module32NextW(h,ref e);}
   int error=Marshal.GetLastWin32Error();if(error!=18)throw new System.ComponentModel.Win32Exception(error,"native_module_enumeration_failed");
   throw new InvalidOperationException("adapter_module_not_loaded");
 ...
 [DllImport("user32.dll",CharSet=CharSet.Unicode,SetLastError=true)]static extern IntPtr SendMessageTimeoutW(IntPtr window,uint message,UIntPtr wparam,IntPtr lparam,uint flags,uint timeout,out UIntPtr result);
  // Qualified gfxz.dat: 990070 returns *(17C89AC); 98FD50 reads router[0].
  // Read only: never call the lazy getter or write a selector to fabricate readiness.
  public static int RouterSelector(int pid,string expectedExe){using(var p=Process.GetProcessById(pid)){
   if(!String.Equals(p.MainModule.FileName,expectedExe,StringComparison.OrdinalIgnoreCase)||p.MainModule.BaseAddress.ToInt64()!=0x400000)throw new InvalidOperationException("client_identity_mismatch");
   IntPtr h=OpenProcess(0x410,false,pid);if(h==IntPtr.Zero)throw new InvalidOperationException("native_process_access_failed");
   try{byte[] b=new byte[4];UIntPtr got;if(!ReadProcessMemory(h,new IntPtr(0x17C89AC),b,(UIntPtr)4,out got)||got.ToUInt64()!=4)return -1;
    uint router=BitConverter.ToUInt32(b,0);if(router<0x10000)return -1;
    if(!ReadProcessMemory(h,new IntPtr((long)router),b,(UIntPtr)4,out got)||got.ToUInt64()!=4)return -1;
    return BitConverter.ToInt32(b,0);
   }finally{CloseHandle(h);}
  }}
  public static bool RoleReady(int pid,string expectedExe){using(var p=Process.GetProcessById(pid)){
  if(!String.Equals(p.MainModule.FileName,expectedExe,StringComparison.OrdinalIgnoreCase)||p.MainModule.BaseAddress.ToInt64()!=0x400000)throw new InvalidOperationException("client_identity_mismatch");
  IntPtr h=OpenProcess(0x410,false,pid);if(h==IntPtr.Zero)throw new InvalidOperationException("native_process_access_failed");
  try{byte[] b=new byte[4];UIntPtr got;if(!ReadProcessMemory(h,new IntPtr(0x17C86F0),b,(UIntPtr)4,out got)||got.ToUInt64()!=4||BitConverter.ToUInt32(b,0)<0x10000)return false;}finally{CloseHandle(h);}
  // Allocation precedes table/resource initialization on the UI thread. A
  // non-null pointer alone must not release the server's first player profile.
  IntPtr window=p.MainWindowHandle;uint owner;if(window==IntPtr.Zero)return false;
  GetWindowThreadProcessId(window,out owner);if(owner!=(uint)pid)return false;
  UIntPtr result;return SendMessageTimeoutW(window,0,UIntPtr.Zero,IntPtr.Zero,3,250,out result)!=IntPtr.Zero;
 }}
 ...
 public static void WaitInstalled(int pid,string exe,string dll,uint mask,int seconds,Action<uint[]> progress=null){
  DateTime deadline=DateTime.UtcNow.AddSeconds(seconds);
  do{uint code=Invoke(pid,exe,dll,"KkNativeCloudInstall",null);if(code!=0&&code!=126&&code!=170)throw new System.ComponentModel.Win32Exception((int)code,"native_adapter_install_failed");
   ...
   if(progress!=null){var fields=new uint[11];for(int i=0;i<11;i++)fields[i]=BitConverter.ToUInt32(status,i*4);progress(fields);}
