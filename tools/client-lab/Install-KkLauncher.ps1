...
[IO.File]::WriteAllText((Join-Path $destination 'launcher.json'),($settings|ConvertTo-Json -Depth 6),$utf8)
if($DesktopShortcut){
 $shell=New-Object -ComObject WScript.Shell
 $link=$shell.CreateShortcut((Join-Path ([Environment]::GetFolderPath('Desktop')) '功夫小子.lnk'))
 if($link.TargetPath-and$link.TargetPath-notin@($entry,(Join-Path $client '开始游戏.cmd'))){throw 'Existing desktop shortcut points elsewhere; installation kept it unchanged'}
