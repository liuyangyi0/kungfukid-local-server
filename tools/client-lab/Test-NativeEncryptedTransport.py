...
async def main(model):
    ...
    async def accept(reader,writer):
        tasks.add(asyncio.current_task());state=None;pending_tail=None
        try:
            while True:
                ...
                if plain==b'peek':
                    first=state.seal(b'head');pending_tail=state.seal(b'tail')
                    writer.write(first+pending_tail[:32]);await writer.drain();continue
                if plain==b'next' and pending_tail is not None:
                    writer.write(pending_tail[32:]);await writer.drain();break
    ...
    try:
        ...
        child=await asyncio.create_subprocess_exec(str(pathlib.Path(model).resolve(strict=True)),'--peek-records',str(server.sockets[0].getsockname()[1]),stdout=asyncio.subprocess.PIPE,stderr=asyncio.subprocess.PIPE,creationflags=0x08000000)
        out,err=await asyncio.wait_for(child.communicate(),15)
        assert child.returncode==0,(child.returncode,err.decode(errors='replace'))
        assert not failures,failures
        assert observed=={'tcp':100008,'udp':1},observed
        print(out.decode().strip())
    finally:
