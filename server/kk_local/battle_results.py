...
from .lab_settlement import (ReportDisagreement, decode_report, consensus_survival,
                             consensus_elimination, consensus_reborn, result_payload)
...
class BattleResults:
    ...
    def handle_lab_result(self, engine, c, message):
        ...
        if room.request[46]==16:
            ...
            except ReportDisagreement:
                # Public non-ranked matches cannot trust either peer's alive
                # state when their complete, well-formed snapshots disagree. Finish the
                # native result/ACK lifecycle without choosing a winner or
                # granting permanent rewards. Malformed reports still fail.
                if hub.public_policy is None or hub.permanent_battle_rewards_allowed:
                    raise
                if hub._battle.metrics is not None:
                    hub._battle.metrics.counts['settlement_hp_disagreement_safe_draw']+=1
                outcomes={participant:0 for participant in groups}
        else:
