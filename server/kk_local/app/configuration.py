...
PATH_OPTIONS=frozenset(('database','client_root','map_client_root','role_ready_file','runtime',
                        'events','log','report','grant_plan','experimental_multi_account_config','config','auth_certificate','auth_key','security_policy','combat_skill_xml','combat_receipt_policy'))
...
def validate_options(mode,args):
    ...
    if mode=='offline':
        ...
        if getattr(args,'combat_receipt_policy',None):
            if getattr(args,'combat_skill_xml',None):raise ValueError('choose one combat policy source')
            if str(Path(args.combat_receipt_policy).resolve()).casefold() in outputs:raise ValueError('public output overlaps combat policy')
    elif mode=='native':
