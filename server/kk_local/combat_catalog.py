...
class CombatCatalog:
    ...
    def receipt_projection(self):
        """Minimal server policy, not an export of SkillProperty XML/effects."""
        return dict(schema='kk-combat-receipt-policy-v1',
                    safe_receipt_ids=sorted(self.safe_receipt_ids),
                    guard_break_ids=sorted(self.guard_break_ids & self.safe_receipt_ids))

    @classmethod
    def from_projection_file(cls,path):
        source=Path(path)
        if not 1<=source.stat().st_size<=128*1024:raise ValueError('combat receipt policy size')
        def unique(pairs):
            result={}
            for key,value in pairs:
                if key in result:raise ValueError('duplicate combat receipt policy key')
                result[key]=value
            return result
        data=json.loads(source.read_text(encoding='utf-8-sig'),object_pairs_hook=unique)
        if (not isinstance(data,dict) or set(data)!={'schema','safe_receipt_ids','guard_break_ids'} or
                data['schema']!='kk-combat-receipt-policy-v1'):
            raise ValueError('combat receipt policy schema')
        lists=[]
        for key in ('safe_receipt_ids','guard_break_ids'):
            ids=data[key]
            if (not isinstance(ids,list) or len(ids)>100000 or
                    any(type(value) is not int or not 0<value<=0x7fffffff for value in ids) or
                    ids!=sorted(set(ids))):
                raise ValueError('combat receipt policy identities')
            lists.append(ids)
        safe,guard=lists
        if not set(guard)<=set(safe):raise ValueError('guard break outside safe receipt policy')
        return cls(safe,source=str(source),guard_break_ids=guard)
