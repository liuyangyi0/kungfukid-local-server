...
class NativeDatagrams(asyncio.DatagramProtocol):
    ...
    def datagram_received(self,data,peer):
        ...
        try:
            ...
            s.rejections.add('identity_or_phase')
            if s.metrics:s.metrics.datagram_rejection(exc)
        except (ProtocolError,ValueError) as exc:
            s.rejections.add('native_shape')
            if s.metrics:s.metrics.datagram_rejection(exc)
        finally:
...
class NativeService:
    def game_read_remaining(self,grant,phase,last_rx,profile_sent_at,now):
        # Already authenticated bootstrap may wait for client preparation only
        # within its original one-use grant deadline. This grants no privilege.
        if self.public_policy and grant is not None and not grant.ready and phase==Phase.BOOTSTRAP:
            return grant.admission_deadline-now
        anchor=max(last_rx,profile_sent_at if profile_sent_at is not None else last_rx)
        return self.idle_seconds-(now-anchor)
    def __init__(self,admission,*,game_port=0,udp_port=0,sdk_port=0,udp_login_verifier=None,
        ...
        self.relay=NativeRelay(admission,emit=self.send_udp,event=lambda **r:self.rejections.add('direct_not_advertised'),metrics=metrics)
    ...
    async def game(self,reader,writer,lease=None):
        ...
        raw_writer=writer
        last_rx=time.monotonic();accepted_at=last_rx;rate_start=last_rx;rate_count=0;partial_since=None;profile_sent_at=None
        active_message_id=None
        admission_limit=self.public_policy.admission_seconds if self.public_policy else 10
        async def send(messages):
            nonlocal profile_sent_at
            ...
            try:
                ...
                async with lock:
                    for message in messages:
                        ...
                        writer.write(data)
                        if self.public_policy and grant is not None and message.id==1151 and profile_sent_at is None:
                            profile_sent_at=time.monotonic()
                        if self.metrics and message.id in self.metrics.HANDSHAKE_IDS:
                            self.metrics.event(dict(event='native_handshake_frame',direction='tx',id=message.id))
        ...
        async def pulse():
            ...
            while True:
                ...
                if e.delivery_failed:writer.close();return
                profile_replies=e.profile_ready(c,grant.ready)
                if profile_replies:await send(profile_replies)
                if self.game_read_remaining(grant,c.phase,last_rx,profile_sent_at,time.monotonic())<=0 or e.delivery_failed:writer.close();return
                if not self.public_policy:self.admission.hub.expire()
                e.poll_battle_clock(c)
                replies=e.take_pending(c)
        ...
        try:
            ...
            if lease:lease.promote(outer.uid)
            while True:
                timeout=max(.001,admission_limit-(time.monotonic()-accepted_at)) if grant is None else max(.001,self.game_read_remaining(grant,c.phase,last_rx,profile_sent_at,time.monotonic()))
                ...
                try:data=await asyncio.wait_for(reader.read(65536),timeout)
                except asyncio.TimeoutError:
                    # Preparation/profile delivery can move the one-time anchor
                    # while read() is pending, never the partial-frame deadline.
                    if grant is not None and partial_since is None and self.game_read_remaining(grant,c.phase,last_rx,profile_sent_at,time.monotonic())>0:continue
                    raise
                ...
                for message in decoder.feed(data):
                    ...
                    if self.metrics and message.id in self.metrics.HANDSHAKE_IDS:
                        self.metrics.event(dict(event='native_handshake_frame',direction='rx',id=message.id))
            ...
            if self.metrics and isinstance(exc,ProtocolError):
                self.metrics.protocol_rejection(active_message_id,exc)
