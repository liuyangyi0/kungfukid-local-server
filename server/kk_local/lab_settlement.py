...
class ReportDisagreement(ProtocolError):
    """Well-formed peers disagree on who is alive; not a malformed report."""
...
def report_validation_facts(payload,roster,room_number,serial):
    """Bounded booleans for diagnostics, no raw rows, names or identities."""
    if len(payload)!=696:return dict(length_valid=False)
    rows=[]
    for slot in range(8):
        raw=payload[slot*87:(slot+1)*87];uid=struct.unpack_from('<Q',raw,29)[0]
        maximum,hp=struct.unpack_from('<HH',raw)
        number,epoch=struct.unpack_from('<II',raw,67)
        rows.append(dict(slot=slot,occupied=slot in roster,all_zero=not any(raw),identity_match=uid==roster.get(slot,0),
                         room_match=number==room_number,serial_match=epoch==serial,maximum_positive=maximum>0,hp_within_reported_max=hp<=maximum))
    return dict(length_valid=True,slots=rows)


def consensus_survival(reports, teams):
    """Local policy only: all peers must agree on each fighter's alive state.
...
Numerical HP can differ after a missed replication event without changing
the survival winner; requiring exact HP would turn a clear knockout into a draw.
"""
    ...
    for report in observations[1:]:
        if (set(report) != set(baseline) or any(
                (report[u].hp > 0) != (baseline[u].hp > 0) for u in baseline)):
            raise ReportDisagreement('settlement peer survival disagreement')
