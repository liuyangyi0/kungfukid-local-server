...
class PublicBattleDispatchTests(unittest.TestCase):
    ...
    def test_hit_delivery_counters_distinguish_received_and_forwarded(self):
        self.prepare();metrics=Metrics();self.h._battle.metrics=metrics
        hit=self.hit()
        self.udp(hit)
        self.assertEqual(self.drain()[0][1],hit)
        self.assertEqual(metrics.counts['battle_8121_received'],1)
        self.assertEqual(metrics.counts['battle_8121_delivered'],1)
        self.udp(hit)
        self.assertEqual(self.drain(),[])
        self.assertEqual(metrics.counts['battle_8121_received'],2)
        self.assertEqual(metrics.counts['battle_8121_delivered'],1)

    def test_three_distinct_combo_hit_reports_all_reach_peer(self):
        self.prepare();metrics=Metrics();self.h._battle.metrics=metrics
        for seq in range(3):
            hit=self.hit(seq=seq)
            self.assertEqual(self.e1.handle(self.c1,hit),[])
            self.assertEqual(self.e2.take_pending(self.c2),[hit])
        self.assertEqual(metrics.counts['battle_8121_received'],3)
        self.assertEqual(metrics.counts['battle_8121_delivered'],3)
        self.assertEqual(metrics.counts['battle_8121_suppressed'],0)
        self.assertEqual(sum(metrics.counts[f'battle_8121_{kind}'] for kind in
                             ('damage_positive','healing','zero_amount')),3)

    def test_hit_suppression_reason_is_bounded_and_distinguishable(self):
        self.prepare();metrics=Metrics();self.h._battle.metrics=metrics
        self.e1.handle(self.c1,self.hit(seq=2));self.e2.take_pending(self.c2)
        self.e1.handle(self.c1,self.hit(seq=1))
        self.assertEqual(metrics.counts['battle_8121_sequence_stale'],1)
        self.e1.room.pending_hit_receipts.clear()
        self.e1.handle(self.c1,self.receipt(seq=3))
        self.assertEqual(metrics.counts['battle_8126_missing_witness'],1)
    ...
    def test_minimal_receipt_projection_roundtrip_and_fail_closed(self):
        with tempfile.TemporaryDirectory() as tmp:
            path=Path(tmp)/'receipt-policy.json'
            original=CombatCatalog.from_xml(ET.fromstring(
                '<SkillProperty><PropertyItem SkillProId="1" DefenceTear="1"/>'
                '<PropertyItem SkillProId="2"><LogicEffects><AttackerUstate/></LogicEffects>'
                '</PropertyItem></SkillProperty>'))
            import json
            path.write_text(json.dumps(original.receipt_projection()),encoding='utf-8')
            restored=CombatCatalog.from_projection_file(path)
            self.assertEqual(restored.receipt_outcomes(1,2,1),(2,4))
            self.assertEqual(restored.receipt_policy(2),'definition_missing')
            data=original.receipt_projection();data['guard_break_ids']=[2]
            path.write_text(json.dumps(data),encoding='utf-8')
            with self.assertRaises(ValueError):CombatCatalog.from_projection_file(path)
